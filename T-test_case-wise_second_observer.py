...
if __name__ == "__main__":
    ...
    gt_root0 = './data/Data_4classes_disc_testDataset_second_observer/npy_first_observer/SegmentationLabel'
    [casewise_PAs0,casewise_IoUs0,casewise_DSs0,casewise_Sensitivitys0,casewise_Precisions0] = count_case_accuracy(pred_root=sec_obs_root,ground_truth_root=gt_root0,n_cls=n_cls)
    
    auto_pred_root = './data/Data_4classes_disc/data_npy_20181213/5-fold/soft_ensemble_predicted_masks'#soft ensmeble
    gt_root1 = './data/Data_4classes_disc/data_npy_20181213/test/SegmentationLabel'
    [casewise_PAs1,casewise_IoUs1,casewise_DSs1,casewise_Sensitivitys1,casewise_Precisions1] = count_case_accuracy(pred_root=auto_pred_root,ground_truth_root=gt_root1,n_cls=n_cls)
    
    h,w = 270,180
    savefile_name = os.path.join(sec_obs_root, 'second_observer_comparision_mask.png')
    draw_mask_compared_figure(pred_root=sec_obs_root,ground_truth_root=gt_root0,n_cls=n_cls,h=h,w=w,savefile_name=savefile_name) 
    
    savefile_name = os.path.join(auto_pred_root, 'soft_ensemble_comparision_mask.png')
    draw_mask_compared_figure(pred_root=auto_pred_root,ground_truth_root=gt_root1,n_cls=n_cls,h=h,w=w,savefile_name=savefile_name) 
    
    # 统计第二观察者混淆矩阵
    y_true, y_pred = flatten_dataset(pred_root=sec_obs_root,ground_truth_root=gt_root0,n_cls=n_cls)
    ...
    plt.show()
    # 数据集上的准确率和敏感度
    dataset_Precisions_sec_obs = precision_score(y_true=y_true, y_pred=y_pred, average=None)
    dataset_Sensitivitys_sec_obs = recall_score(y_true=y_true, y_pred=y_pred, average=None)
    dataset_f1scores_sec_obs = f1_score(y_true=y_true, y_pred=y_pred, average=None)
    
    # 统计soft ensemble混淆矩阵
    y_true, y_pred = flatten_dataset(pred_root=auto_pred_root,ground_truth_root=gt_root1,n_cls=n_cls)
    cm = confusion_matrix(y_true,y_pred)
    plot_confusion_matrix(cm.astype(int),labels=labels,title='Confusion Matrix',cmap=plt.cm.spring,dtype='int')#cmap=plt.cm.binary#plt.cm.spring
    plt.show()
    cn_normalized = cm.astype('float')/cm.sum(axis=1)
    ...
    # soft ensemble和第二观察者Dice Score的配对样本t检验: ttest_rel
    Ttest_DSs,DSs0_men,DSs0_std,DSs1_men,DSs1_std = Ttest_rel(X0=casewise_DSs0,X1=casewise_DSs1)
    print(Ttest_DSs,'\n',DSs0_men,'\n',DSs0_std,'\n',DSs1_men,'\n',DSs1_std)
