...
"""
# 神经根分割项目
# 读取的2018111数据，一共有52例，背景/骨头/神经根 共三类数据
# 在unet_3Dn.py做如下改进，，将神经根和硬膜囊合为一个类别
"""
...
def weighted_softmax_cross_entropy_loss(logits, labels, weights, n_cls):
...
def getUnet3D(img_frame,img_rows,img_cols,img_channels,n_cls):
    ...
    up5 = Conv3D(512, 2, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(UpSampling3D(size = (2,2,2))(drop4))
    print("up5 shape:",up5.shape)
    merge5 = Concatenate(axis=4)([conv3,up5])
    conv5 = Conv3D(256, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(merge5)
    conv5 = Conv3D(256, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(conv5)
    conv5 = BN(axis=-1)(conv5)
    
    up6 = Conv3D(256, 2, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(UpSampling3D(size = (2,2,2))(conv5))
...
def parse_args():
    ...
    parser.add_argument('--n_cls',default=3,
                        help='total number of pixel classes')
    parser.add_argument('--train_dir',default='./data/Data_3classes/data_20181111/data_npy_20181111/5-fold-training&validation/3/training', 
                        help='train data directory')
    parser.add_argument('--validation_dir',default='./data/Data_3classes/data_20181111/data_npy_20181111/5-fold-training&validation/3/validation',
                        help='validation data directory')
    parser.add_argument('--test_dir',default='./data/Data_3classes/data_20181111/data_npy_20181111/test',
    ...
    parser.add_argument('--weights',default=[1,1,20],#background,bone,nerve
    ...
    parser.add_argument('--learning_rate',default=0.0005,
                        help='learning rate')
    parser.add_argument('--training_iters',default=40000,type=int,#50000
                        help='total iteration times of the training')
    parser.add_argument('--validation_iter',default=6,type=int,
                        help='number of cases to be evaluated in a single validation')
    parser.add_argument('--display_step',default=25,type=int,
...
if __name__ == "__main__":
    ...
    test_dir = args.test_dir # 测试数据的路径
...
# =============================================================================
# 
#     iterator_get_trainBatch, init_op_get_trainBatch = get_dataBatch(train_dir,img_frame,image_W,image_H,batch_size)# 定义一个迭代器（读取训练数据）
#     iterator_get_validationBatch, init_op_get_validationBatch = get_validationBatch(validation_dir,img_frame,image_W,image_H,batch_size=1)# 定义一个迭代器（读取验证数据）
#     iterator_get_testBatch, init_op_get_testBatch = get_validationBatch(test_dir,img_frame,image_W,image_H,batch_size=1)# 定义一个迭代器（读取测试数据）
# =============================================================================
    ...
    loss = weighted_softmax_cross_entropy_loss(preds,labels,weights,n_cls)#定义损失函数
    ...
    correct_pred = tf.equal(tf.expand_dims(tf.cast(tf.argmax(preds,4),tf.float32),dim=-1),tf.cast(labels,tf.float32))#tf.euqal：返回相同位置相等的bool值;tf.argmax运算后会降低一个维度，所以要用expand_dims来恢复维度
    ...
    with sess.as_default():
        ...
        sess.run([init_op,init_op_get_trainBatch,init_op_get_validationBatch])
        ...
        while step*batch_size < training_iters:#not coord.should_stop() and 
            try:
                ...
                imgs,masks = sess.run(iterator_get_trainBatch.get_next())#获得输入的batch
                ...
                if step%display_step == 0:#每display_step显示一次训练过程中的loss和accuracy,执行一次validation
                    ...
                    for val_iter in range(validation_iter):
                        image,mask = sess.run(iterator_get_validationBatch.get_next())#获得validation的batch
                ...
                sess.run([init_op_get_trainBatch,init_op_get_validationBatch])
